...
def check_port_scan(src_ip, now):
    ...
    if len(ports) >= PORT_SCAN_THRESHOLD:
        if should_alert(src_ip, "PORT_SCAN", now):
            print(f"[SIGNATURE ALERT] PORT SCAN from {src_ip} — {len(ports)} unique ports in {TIME_WINDOW}s")
    ...
    for port, count in port_counts.items():
        if count >= SYN_FLOOD_THRESHOLD:
            if should_alert(src_ip, f"SYN_FLOOD_{port}", now):
                print(f"[SIGNATURE ALERT] SYN FLOOD from {src_ip} — port {port} hit {count} times in {TIME_WINDOW}s")

# ---- ML-based detection: simplified feature extraction ----
def build_feature_vector(src_ip, now):
    ...
    # Approximate error rate: rapid connections to many different ports
    # with no established session strongly resembles scan/error behavior
...
def check_ml_anomaly(src_ip, now):
    ...
    if prediction == 'attack' and confidence > 0.7:
        if should_alert(src_ip, "ML_ANOMALY", now):
            print(f"[ML ALERT] Anomalous traffic from {src_ip} — confidence {confidence:.2f}")
...
if __name__ == "__main__":
    ...
    print("Starting hybrid detection engine (signature + ML)... Ctrl+C to stop")
    try:
